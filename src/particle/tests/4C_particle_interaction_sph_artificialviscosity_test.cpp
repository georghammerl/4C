...
namespace
{
  ...
  TEST_F(SPHArtificialViscosityTest, ArtificialViscosity)
  {
    ...
    double acc_i_ref[3];
    Particle::ParticleUtils::vec_set_scale(acc_i_ref, (artvisc_i * mass_j * dWdrij * fac), e_ij);

    double acc_j_ref[3];
    Particle::ParticleUtils::vec_set_scale(acc_j_ref, (-artvisc_j * mass_i * dWdrji * fac), e_ij);
