...
/*---------------------------------------------------------------------------*
 | definitions                                                               |
 *---------------------------------------------------------------------------*/
Particle::DEMContactRollingBase::DEMContactRollingBase(const Teuchos::ParameterList& params)
    : params_dem_(params),
      dt_(0.0),
...
void Particle::DEMContactRollingBase::setup(const double& k_normal)
{
  // nothing to do
}

void Particle::DEMContactRollingBase::set_current_step_size(const double currentstepsize)
{
  dt_ = currentstepsize;
}

Particle::DEMContactRollingViscous::DEMContactRollingViscous(const Teuchos::ParameterList& params)
    : Particle::DEMContactRollingBase(params),
...
void Particle::DEMContactRollingViscous::setup(const double& k_normal)
{
  // call base class setup
  DEMContactRollingBase::setup(k_normal);

  // determine rolling contact damping factor
  const double fac = young_ / (1.0 - ParticleUtils::pow<2>(nue_));
  const double c_1 = 1.15344;
  d_rolling_fac_ = (1.0 - e_) / (c_1 * std::pow(fac, 0.4) * std::pow(v_max_, 0.2));
}

void Particle::DEMContactRollingViscous::effective_radius_particle(
...
void Particle::DEMContactRollingViscous::relative_rolling_velocity(const double& r_eff,
    const double* normal, const double* angvel_i, const double* angvel_j,
    double* v_rel_rolling) const
{
  ParticleUtils::vec_set_cross(v_rel_rolling, angvel_i, normal);
  if (angvel_j) ParticleUtils::vec_add_cross(v_rel_rolling, normal, angvel_j);
}

void Particle::DEMContactRollingViscous::rolling_contact_moment(double* gap_rolling,
    ...
    double* rollingcontactmoment) const
{
  ...
  ParticleUtils::vec_set_cross(rollingcontactmoment, rollingcontactforce, normal);
  ParticleUtils::vec_scale(rollingcontactmoment, r_eff);
}

void Particle::DEMContactRollingViscous::rolling_potential_energy(
    const double* gap_rolling, double& rollingpotentialenergy) const
{
  rollingpotentialenergy = 0.0;
}

Particle::DEMContactRollingCoulomb::DEMContactRollingCoulomb(const Teuchos::ParameterList& params)
    : Particle::DEMContactRollingBase(params), k_rolling_(0.0)
{
  // empty constructor
}

void Particle::DEMContactRollingCoulomb::setup(const double& k_normal)
{
  // call base class setup
  DEMContactRollingBase::setup(k_normal);
...
void Particle::DEMContactRollingCoulomb::effective_radius_particle(
    const double* radius_i, const double* radius_j, const double& gap, double& r_eff) const
{
  if (radius_j)
    r_eff =
...
void Particle::DEMContactRollingCoulomb::relative_rolling_velocity(const double& r_eff,
    const double* normal, const double* angvel_i, const double* angvel_j,
    double* v_rel_rolling) const
{
  ParticleUtils::vec_set_cross(v_rel_rolling, normal, angvel_i);
  if (angvel_j) ParticleUtils::vec_add_cross(v_rel_rolling, angvel_j, normal);

  ParticleUtils::vec_scale(v_rel_rolling, r_eff);
}

void Particle::DEMContactRollingCoulomb::rolling_contact_moment(double* gap_rolling,
    bool& stick_rolling, const double* normal, const double* v_rel_rolling, const double& m_eff,
    const double& r_eff, const double& mu_rolling, const double& normalcontactforce,
    double* rollingcontactmoment) const
{
  ...
  const double old_length = ParticleUtils::vec_norm_two(gap_rolling);

  // compute projection of rolling gap onto current normal at time n+1
  ParticleUtils::vec_add_scale(gap_rolling, -ParticleUtils::vec_dot(normal, gap_rolling), normal);

  // compute length of rolling gap at time n+1
  const double new_length = ParticleUtils::vec_norm_two(gap_rolling);

  // maintain length of rolling gap equal to before the projection
  if (new_length > 1.0e-14)
    ParticleUtils::vec_set_scale(gap_rolling, old_length / new_length, gap_rolling);

  // update of elastic rolling displacement if stick is true
  if (stick_rolling == true) ParticleUtils::vec_add_scale(gap_rolling, dt_, v_rel_rolling);

  // compute rolling contact force (assume stick-case)
  double rollingcontactforce[3];
  ParticleUtils::vec_set_scale(rollingcontactforce, -k_rolling_, gap_rolling);
  ParticleUtils::vec_add_scale(rollingcontactforce, -d_rolling, v_rel_rolling);

  // compute the norm of the rolling contact force
  const double norm_rollingcontactforce = ParticleUtils::vec_norm_two(rollingcontactforce);
  ...
  else
  {
    ...
    // compute rolling contact force
    ParticleUtils::vec_set_scale(rollingcontactforce,
    ...
    ParticleUtils::vec_set_scale(gap_rolling, -inv_k_rolling, rollingcontactforce);
    ParticleUtils::vec_add_scale(gap_rolling, -inv_k_rolling * d_rolling, v_rel_rolling);
  }
...
void Particle::DEMContactRollingCoulomb::rolling_potential_energy(
    const double* gap_rolling, double& rollingpotentialenergy) const
{
  rollingpotentialenergy = 0.5 * k_rolling_ * ParticleUtils::vec_dot(gap_rolling, gap_rolling);
}
