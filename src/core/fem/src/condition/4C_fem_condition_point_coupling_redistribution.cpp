    ...
    Core::FE::Discretization& discret)
{
  ...
  for (size_t inner = 0; inner < all_point_coupling_conditions.size(); ++inner)
  {
    ...
    int master_id = *(overlapping_conditions[inner].begin());
    ...
    Core::FE::Discretization& discret)
{
  ...
  Core::LinAlg::Map new_row_node_map(discret.num_global_nodes(), new_row_nodes.size(),
      new_row_nodes.data(), 0, Core::Communication::as_epetra_comm(discret.get_comm()));
  ...
  // export nodal graph to new row node layout
  {
    const Epetra_Export exporter(
        discret.node_row_map()->get_epetra_map(), new_row_node_map.get_epetra_map());
  ...
  const Epetra_BlockMap new_col_node_block_map = node_graph.col_map();
  const Core::LinAlg::Map new_col_node_map(-1, new_col_node_block_map.NumMyElements(),
      new_col_node_block_map.MyGlobalElements(), 0,
